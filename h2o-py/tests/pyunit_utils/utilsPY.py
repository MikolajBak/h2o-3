...
def list_all_files(paths, filter_=None, recursive=False):
    """
    ...
    for path in paths:
        if os.path.isdir(path):
            for root_dir, sub_dirs, files in os.walk(path):
                ...
                if recursive:
                    for sd in sub_dirs:
                        all_files.extend(list_all_files(sd, filter_, recursive))
...
def load_utilities(test_file=None):
    ...
    if test_file is None:
        ...
        folders.append(os.path.join(os.path.dirname(__file__), ".."))
        recursive = True
    else:
